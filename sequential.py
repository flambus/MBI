...
def get_info(lib, device_id1):
    print("\nGet device info")
    x_device_information = device_information_t()
    result = lib.get_device_information(device_id1, byref(x_device_information))
...
def get_status(lib, device_id1):
    print("\nGet status")
    x_status = status_t()
    result = lib.get_status(device_id1, byref(x_status))
    print("Result: " + repr(result))
    if result == Result.Ok:
...
def test_get_position(lib, device_id1):
    print("\nRead position")
    x_pos = get_position_t()
    result = lib.get_position(device_id1, byref(x_pos))
    print("Result: " + repr(result))
    if result == Result.Ok:
        print("Position: {0} steps, {1} microsteps".format(x_pos.Position, x_pos.uPosition))
    return x_pos.Position, x_pos.uPosition

def move(lib, device_id1, distance, udistance):
    print("\nGoing to {0} steps, {1} microsteps".format(distance, udistance))
    result = lib.command_move(device_id1, distance, udistance)
    print("Result: " + repr(result))

def set_microstep_mode_256(lib, device_id1):
    print("\nSet microstep mode to 256")
    # Create engine settings structure
    eng = engine_settings_t()
    # Get current engine settings from controller
    result = lib.get_engine_settings(device_id1, byref(eng))
    ...
    result = lib.set_engine_settings(device_id1, byref(eng))
    # Print command return status. It will be 0 if all is OK
    print("Write command result: " + repr(result))

def wait_for_stop(lib, device_id1, interval):
    print("\nWaiting for stop")
    result = lib.command_wait_for_stop(device_id1, interval)
    print("Result: " + repr(result))

def get_speed(lib, device_id1):
    print("\nGet speed")
    # Create move settings structure
    mvst = move_settings_t()
    # Get current move settings from controller
    result = lib.get_move_settings(device_id1, byref(mvst))
    # Print command return status. It will be 0 if all is OK
    print("Read command result: " + repr(result))

    return mvst.Speed

def set_speed(lib, device_id1, speed):
    print("\nSet speed")
    # Create move settings structure
    mvst = move_settings_t()
    # Get current move settings from controller
    ...
    result = lib.set_move_settings(device_id1, byref(mvst))
    # Print command return status. It will be 0 if all is OK
    print("Write command result: " + repr(result))
...
get_info(lib, device_id1)
get_status(lib, device_id1)
set_microstep_mode_256(lib, device_id1)
set_speed(lib, device_id1, speed)

move(lib, device_id1, start, ustart)
wait_for_stop(lib, device_id1, 100)
...
while camera.IsGrabbing():
    ...
    if grabResult.GrabSucceeded():
        ...
        imgG = cv.cvtColor(imgCV, cv.COLOR_BGR2GRAY)

        ret, thresh = cv.threshold(imgG, 73, 200, cv.THRESH_BINARY_INV)
        kernal = np.ones((6, 6), np.uint8)

        dilation = cv.dilate(thresh, kernal, iterations=2)

        edges = cv.Canny(dilation, 100, 200)

        contours, hierarchy = cv.findContours(dilation, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)

        objects = str(len(contours))

        wait_for_stop(lib, device_id1, 120000)

        startpos, ustartpos = test_get_position(lib, device_id1)

        move(lib, device_id1, startpos + steps, ustartpos)
